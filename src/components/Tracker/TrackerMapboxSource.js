 ...
 */
class TrackerMapboxSource extends MapboxLayer {
  ...
  constructor(options = {}) {
    ...
    this.url =
      'https://backend1.tracker.geops.de' || 'https://tracker.geops.io';
  ...
  animateTrajectories() {
    this.mbMap
      .getSource('point')
      .setData(this.tracker.renderGeojsonTrajectory(this.currTime));

    // Request the next frame of the animation.
    requestAnimationFrame(this.animateTrajectories.bind(this));
  }
  ...
  init(map) {
    ...
    this.mbMap.on('load', () => {
      ...
      this.mbMap.addLayer({
        ...
        type: 'circle',
        paint: {
          'circle-radius': ['match', ['get', 'type'], '0', 12, '3', 8, 11],
  ...
  getUrlParams(extraParams = {}) {
    ...
    const btime = TrackerMapboxSource.getTimeString(now);
    ...
    const params = {
      ...
      ory: ext[3],
      btime,
      etime: TrackerMapboxSource.getTimeString(this.later),
      date: TrackerMapboxSource.getDateString(now),
      rid: 1,
      a: 1,
      cd: 1,
      nm: 1,
  ...
  fetchTrajectory(id) {
    const params = this.getUrlParams({
      id,
      time: TrackerMapboxSource.getTimeString(new Date()),
    });
...
export default TrackerMapboxSource;
